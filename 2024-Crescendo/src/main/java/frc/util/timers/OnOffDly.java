...
public class OnOffDly {
  ...
  public OnOffDly(long onDly, long offDly) {
    setTm(onDly, offDly);
  }
  // Constructor for a timer in mSeconds for both.
  public OnOffDly(long delay) {
    setTm(delay, delay);
  }
  ...
  // Constructor for a timers in mSeconds.
  public OnOffDly(double onDly, double offDly) {
    setTm((long) (onDly * 1000.0), (long) (offDly * 1000.0));
  }
  // Constructor for a timer in mSeconds for both.
  public OnOffDly(double delay) {
    setTm((long) (delay * 1000.0), (long) (delay * 1000.0));
  }
  ...
  public boolean get(boolean trigger) {
    ...
    // 2 - Delay on/off, delay on, AND delay off.
    statOODly = (trigger && currentMSec > delayOnTmr) || (!trigger && currentMSec < delayOffTmr);
