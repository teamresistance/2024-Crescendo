...
/**
 * Class for the CANSparkMax PID RPM control. In order to use PID functionality for a controller, a
 * SparkPIDController object is constructed by calling the getPIDController() method on an existing
 * CANSparkMax object.
 */
public class MotorPID_NEO {
  ...
  /**
   * Constructor. In order to use PID functionality for a controller, a SparkPIDController object is
   ...
   */
  public MotorPID_NEO(CANSparkMax _m_motor, String _sdbTag) {
   ...
   */
  public MotorPID_NEO(CANSparkMax _m_motor, String _sdbTag, double[] _parms) {
    m_pidController = _m_motor.getPIDController();
    sdbTag += _sdbTag + "/Mtr_" + _m_motor.getDeviceId() + "/"; // Used to individualize sdb
    for (int i = 0; i < _parms.length; i++) {
      switch (i) {
        ...
        case 6:
          ...
          break;
        default:
          System.out.println("Too many parms " + i + " " + _parms[i]);
      }
    }
    init();
  }
   ...
   *
   * <p>The first parameter is the value of the set point, whose units vary depending on the control
   * type set in the second parameter.
   *
  ...
  public double getMin() {
    return m_pidController.getOutputMin();
  }
  /**
   * @return pid controller outputMax value
   */
  public double getMax() {
    return m_pidController.getOutputMax();
  }
}
