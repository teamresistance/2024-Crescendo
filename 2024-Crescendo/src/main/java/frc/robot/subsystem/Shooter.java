...
public class Shooter {
	...
	public static void init() {
		...
		clsDistToFPS =
			new double[][]{
				{2.05 + 1., 2.35 + 1.6, 2.70 + 2.0, 3.00 + 2.1, 3.32 + 2.391},
				{fpsMax, fpsMax, fpsMax, fpsMax, 35.0}
			}; // Segmented Line close
		farDistToFPS =
			new double[][]{
				{3.30 + 2.391, 5.80 + 2.391, 9.99 + 2.391, 10.0},
				{fpsMax, 50.5, 50.5, fpsMax}
			}; // Segmented Line far
		farDistDB =
	...
	private static void calcShotDist() {
		...
		if (!shtrTestActive) {
			shotIsFar = !(distToTarget <= 4);
			
			if (!shotIsFar) {
				v =
					((((0.14 * Math.pow(x, 4)) - (1.625 * Math.pow(x, 3))) + (6.854 * Math.pow(x, 2)))
						- (13.375 * x))
						+ 33;
			} else {
				v =
					((((-0.06 * Math.pow(x, 4)) + (1.383 * Math.pow(x, 3))) - (11.3886 * Math.pow(x, 2)))
						+ (40.13279 * x))
						- 24.167;
			}
			shtrAFPS_SP = v;
